...
impl App {
    ...
    fn new(url: String) -> Result<Self, anyhow::Error> {
        ...
        //  rtph264depay ! vaapih264dec ! vaapipostproc ! video/x-raw,width=1920,height=1080 ! vaapih264enc rate-control=cbr bitrate=1000 ! \
        ...
        // )?;
          ...
          let pipeline = gst::parse_launch(
                &"webrtcbin name=sendrecv stun-server=stun://stun.l.google.com:19302 \
                videotestsrc pattern=ball ! video/x-raw,width=640,height=480 ! videoconvert ! queue !
                vaapih264enc ! rtph264pay ! queue ! application/x-rtp,media=video,encoding-name=H264,payload=96 ! sendrecv."
                .to_string(),
            )?;
